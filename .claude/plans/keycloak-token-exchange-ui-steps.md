...
TOKEN=$(curl -s -X POST "http://localhost:8190/auth/realms/master/protocol/openid-connect/token" \
...
RESULT=$(curl -s -X POST "http://localhost:8190/auth/realms/tamshai-corp/protocol/openid-connect/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "client_id=mcp-integration-runner" \
  -d "client_secret=$SECRET_ENCODED" \
