...
POST http://localhost:8190/auth/realms/tamshai-corp/protocol/openid-connect/token
...
- Tokens are signed by Keycloak accessed at `http://localhost:8190/auth` (external)
...
curl http://localhost:8190/auth/realms/tamshai-corp/protocol/openid-connect/certs | jq .
...
- Dev environment should use `http://localhost:8190` or `http://keycloak:8080`

**Test**:

```bash
# Check realm configuration
curl -s "http://localhost:8190/auth/admin/realms/tamshai-corp" \
...
```bash
# From host
curl -s http://localhost:8190/auth/realms/tamshai-corp/protocol/openid-connect/certs | jq .keys[0].kid
...
```bash
# Get token
SERVICE_TOKEN=$(curl -s -X POST http://localhost:8190/auth/realms/tamshai-corp/protocol/openid-connect/token \
  -d 'client_id=mcp-integration-runner' \
  -d 'client_secret=cQFv7tO4FZPQS6my5YF+cRD7Z3XJJ6owuZWbhqdFXuc=' \
  -d 'grant_type=client_credentials' | jq -r '.access_token')

BOB_TOKEN=$(curl -s -X POST http://localhost:8190/auth/realms/tamshai-corp/protocol/openid-connect/token \
  -d 'client_id=mcp-integration-runner' \
  -d 'client_secret=cQFv7tO4FZPQS6my5YF+cRD7Z3XJJ6owuZWbhqdFXuc=' \
...
curl -v -X POST http://localhost:3110/api/mcp/finance/tools/approve_expense_report \
...
1. **Verify JWKS Key Consistency**
   - Compare keys from `localhost:8190` vs `keycloak:8080`
