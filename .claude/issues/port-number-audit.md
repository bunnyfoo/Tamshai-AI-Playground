...
Audit of files with hardcoded `localhost:` port numbers that should use GitHub variables from the `DEV_PG_*` table.
...
|---------|----------|--------------|----------|
| Keycloak | 8180 | DEV_PG_KEYCLOAK | 8190 |
| Kong Proxy | 8100 | DEV_PG_KONG_PROXY | 8110 |
| Kong Admin | 8101 | DEV_PG_KONG_ADMIN | 8111 |
| MCP Gateway | 3100 | DEV_PG_MCP_GATEWAY | 3110 |
| MCP HR | 3101 | DEV_PG_MCP_HR | 3111 |
| MCP Finance | 3102 | DEV_PG_MCP_FINANCE | 3112 |
| MCP Sales | 3103 | DEV_PG_MCP_SALES | 3113 |
| MCP Support | 3104 | DEV_PG_MCP_SUPPORT | 3114 |
| MCP Journey | 3105 | DEV_PG_MCP_JOURNEY | 3115 |
| MCP Payroll | 3106 | DEV_PG_MCP_PAYROLL | 3116 |
| MCP Tax | 3107 | DEV_PG_MCP_TAX | 3117 |
| MCP UI | 3108 | DEV_PG_MCP_UI | 3118 |
| PostgreSQL | 5433 | DEV_PG_POSTGRES | 5443 |
| MongoDB | 27018 | DEV_PG_MONGODB | 27028 |
| Redis | 6379 | DEV_PG_REDIS | 6390 |
| Elasticsearch | 9201 | DEV_PG_ELASTICSEARCH | 9211 |
...
**Action**: Update all port references to match DEV_PG_* variables
...
**Should be**: Use environment variable with fallback to 8190
**Impact**: Unit test consistency (mocked, but should match actual ports)
**Recommendation**:

```typescript
const testKeycloakUrl = process.env.KEYCLOAK_URL || 'http://localhost:8190';
config = {
...
**Should be**: `VITE_API_BASE_URL=http://localhost:8110/api` (DEV_PG_KONG_PROXY)
...
**Should be**: `const mcpHrUrl = process.env.MCP_HR_URL || 'http://localhost:3111';`

### 4. `services/mcp-gateway/src/__tests__/setup.ts`

**Line**: 9
**Current**: `process.env.REDIS_URL = 'redis://localhost:6379';`
**Should be**: `process.env.REDIS_URL = 'redis://localhost:6390';` (DEV_PG_REDIS)

### 5. `tests/shared/auth/token-exchange.ts`

**Line**: 233
**Current**: `keycloakUrl: process.env.KEYCLOAK_URL || 'http://localhost:8190/auth',`
**Status**: ✅ **CORRECT** - Already uses 8190
...
✅ `.github/workflows/ci.yml` - **Correctly uses `${{ vars.DEV_PG_* }}`**
✅ `.github/actions/setup-keycloak/action.yml` - **Uses input parameters**

### Shared Test Utilities

✅ `tests/shared/auth/token-exchange.ts` - **Correctly uses env var with 8190 fallback**
...
```typescript
const KEYCLOAK_URL = process.env.KEYCLOAK_URL || 'http://localhost:8190/auth';
const MCP_GATEWAY_URL = process.env.MCP_GATEWAY_URL || 'http://localhost:3110';
const MCP_HR_URL = process.env.MCP_HR_URL || 'http://localhost:3111';
```

**Pros**:
- Works in both local dev and CI
- CI can override with `${{ vars.DEV_PG_* }}`
...
<!-- New -->
Access MCP Gateway at http://localhost:3110 (or $MCP_GATEWAY_PORT if configured)
```
