...
| `MCP_SUPPORT_URL` | No | `http://127.0.0.1:3114` | Docker Compose |
| `KEYCLOAK_CLIENT_SECRET` | No | `test-client-secret` | Keycloak config |
| `REDIS_URL` | No | `redis://127.0.0.1:6390` | Docker Compose |
...
**Env Vars**: `KEYCLOAK_URL` (default: `http://127.0.0.1:8190/auth`), `DEV_USER_PASSWORD`, `CLAUDE_API_KEY` (optional).
...
Ports are derived from `PORT_*` variables in `infrastructure/docker/.env` (generated by Terraform). The `jest.config.js` loads this file via `dotenv` and derives service URLs automatically.
...
- **Correct** (local dev): `http://127.0.0.1:8190/auth/realms/tamshai-corp/...`
- **Incorrect** (local dev): `http://127.0.0.1:8190/realms/tamshai-corp/...` (returns 404)
