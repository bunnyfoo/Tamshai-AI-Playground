...
Bash(cd C:/Users/jcorn/Tamshai-AI-Playground/tests/integration && export KEYCLOAK\_URL=<Redacted\> &&  
export KEYCLOAK\_REALM=<Redacted\> && export GATEWAY\_URL=<Redacted\> && export  
DEV\_USER\_PASSWORD=<Redacted\> && export  
MCP\_GATEWAY\_CLIENT\_SECRET=<Redacted\> && export  
MCP\_INTERNAL\_SECRET=<Redacted\> && export  
MCP\_HR\_URL=<Redacted\> && export MCP\_FINANCE\_URL=<Redacted\> && export  
MCP\_SALES\_URL=<Redacted\> && export MCP\_SUPPORT\_URL=<Redacted\> && export  
KEYCLOAK\_ADMIN\_PASSWORD=<Redacted\> && npm test 2\>&1 | tail -40)  

Environment variables are automatically derived from `infrastructure/docker/.env` which is generated by Terraform from GitHub Secrets and Variables. No manual configuration required.
...
All 8 test users must exist in Keycloak: 
    ...
    curl http://localhost:$PORT_KEYCLOAK/health/ready
    `
...
**Fix**: 
    ...
    ADMIN_TOKEN=$(curl -s -X POST http://localhost:$PORT_KEYCLOAK/realms/master/protocol/openid-connect/token \
    -d "client_id=admin-cli" -d "username=admin" -d "password=admin" -d "grant_type=password" | \
    jq -r '.access_token')

    curl -s "http://localhost:$PORT_KEYCLOAK/admin/realms/tamshai-corp/users" \
