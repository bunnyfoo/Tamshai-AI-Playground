...
declare -A FORBIDDEN_DEFAULTS=(
    ["POSTGRES_PASSWORD"]="postgres_password tamshai_password [REDACTED-DEV-PASSWORD]"
    ["KEYCLOAK_ADMIN_PASSWORD"]="admin"
    ["MONGODB_ROOT_PASSWORD"]="[REDACTED-DEV-PASSWORD] tamshai_password"
    ["MONGODB_PASSWORD"]="[REDACTED-DEV-PASSWORD] tamshai_password"
    ["REDIS_PASSWORD"]=""
...
for script in "${HELPER_SCRIPTS[@]}"; do
    if [[ -f "$script" ]]; then
        if grep -qE "([REDACTED-DEV-PASSWORD]|[REDACTED-DEV-PASSWORD])" "$script"; then
