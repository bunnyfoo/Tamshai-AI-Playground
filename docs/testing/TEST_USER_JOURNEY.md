...
| **TOTP Secret** | `***REDACTED_TOTP***` | Base32 encoded for automated TOTP generation |
...
test('test-user.journey can login and access app', async ({ page }) => {
  ...
  const totp = generateTOTP('***REDACTED_TOTP***');
...
- `TEST_USER_TOTP_SECRET`: `***REDACTED_TOTP***`
